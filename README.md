_This project has been created as part of the 42 curriculum by seoliver._

# Python Exercises
### *10 Modules - 42 School Curriculum*


This project contains a series of exercises that I have done to learn the Python programming language through an autonomous and project-based methodology. This repository includes 10 modules that build upon each other, progressing to the goal of learning object-oriented programming and functional programming.

Inside of each module a readme file contain links to resources to learn the concepts used in the exercises. There are also notes with important checks to take into account after doing the exercises.

## Index

[<img src="module-07/battle.png" align="right">](module-07)

00. [Python Fundamentals](module-00)  🌱
01. [Classes, Object-Oriented Prog.](module-01) 🌳
02. [Errors, Exceptions and Data Engineering](module-02) 💦
03. [Python Collections](module-03) 🎮
04. [Read, Write Digital Files](module-04) 📖
05. [Polymorphic Data Streams](module-05) 🧬
06. [Import Packages and Modules](module-06) ⚗️
07. [Creatures Abstract Architecture](module-07) 🐾
08. [Virtual Environments](module-08) 📊
09. [Pydantic Models & Validation](module-09) 🚀
10. [Functional Programming](module-10) 🪄

## Module contents

| M. | Exercises | Contents |
| :----: | --------: | -------- |
| [00](module-00) | [ex0](module-00/ex0/ft_hello_garden.py), [ex1](module-00/ex1/ft_garden_name.py), [ex2](module-00/ex2/ft_plot_area.py), [ex3](module-00/ex3/ft_harvest_total.py) | print(), input(), int()
|    | [ex4](module-00/ex4/ft_plant_age.py), [ex5](module-00/ex5/ft_water_reminder.py), [ex6](module-00/ex6/ft_count_harvest_iterative.py), [ex7](module-00/ex7/ft_seed_inventory.py) | conditionals, loops, recursion
| [01](module-01) | [ex0](module-01/ex0/ft_garden_intro.py), [ex1](module-01/ex1/ft_garden_data.py), [ex2](module-01/ex2/ft_plant_growth.py), [ex3](module-01/ex3/ft_plant_factory.py), [ex4](module-01/ex4/ft_garden_security.py) | classes, methods, attributes
|    | [ex5](module-01/ex5/ft_plant_types.py), [ex6](module-01/ex6/ft_garden_analytics.py) | inheritance, @staticmethod, @classmethod
| [02](module-02) | [ex0](module-02/ex0/ft_first_exception.py), [ex1](module-02/ex1/ft_raise_exception.py), [ex2](module-02/ex2/ft_different_errors.py) | errors, try/except, raise
|    | [ex3](module-02/ex3/ft_custom_errors.py), [ex4](module-02/ex4/ft_finally_block.py) | custom error classes, finally cleanup
| [03](module-03) | [ex0](module-03/ex0/ft_command_quest.py) | arguments, data parsing
|    | [ex1](module-03/ex1/ft_score_analytics.py), [ex2](module-03/ex2/ft_coordinate_system.py), [ex3](module-03/ex3/ft_achievement_tracker.py), [ex4](module-03/ex4/ft_inventory_system.py) | list, tuple, set, dict
|    | [ex5](module-03/ex5/ft_data_stream.py) | generator, yield
|    | [ex6](module-03/ex6/ft_data_alchemist.py) | comprehension
| [04](module-04) | [ex0](module-04/ex0/ft_ancient_text.py), [ex1](module-04/ex1/ft_archive_creation.py), [ex2](module-04/ex2/ft_stream_management.py) | file open, read, write
|    | [ex3](module-04/ex3/ft_vault_security.py) | with, secure file handling
| [05](module-05) | [ex0](module-05/ex0/data_processor.py), [ex1](module-05/ex1/data_stream.py), [ex2](module-05/ex2/data_pipeline.py) | polymorphism, abstract classes, method overriding
| [06](module-06) | [al0](module-06/ft_alembic_0.py), [1](module-06/ft_alembic_1.py), [2](module-06/ft_alembic_2.py), [3](module-06/ft_alembic_3.py), [4](module-06/ft_alembic_4.py), [5](module-06/ft_alembic_5.py) | import mechanics and package access
|    | [di0](module-06/ft_distillation_0.py), [1](module-06/ft_distillation_1.py), [tr0](module-06/ft_transmutation_0.py), [1](module-06/ft_transmutation_1.py), [2](module-06/ft_transmutation_2.py) | package imports, module aliasing, import structure
|    | [kb0](module-06/ft_kaboom_0.py), [1](module-06/ft_kaboom_1.py) | circular dependencies and import failures
| [07](module-07) | [ex0](module-07/battle.py), [ex1](module-07/capacitor.py), [ex2](module-07/tournament.py) | class factories, interfaces, strategy patterns
| [08](module-08) | [ex0](module-08/ex0/construct.py) | virtual environment, 
|    | [ex1](module-08/ex1/loading.py) | dependency package management, pip, poetry, panda, numpy, matplotlib
|    | [ex2](module-08/ex2/oracle.py) | environment config, dotenv
| [09](module-09) | [ex0](module-09/ex0/space_station.py), [ex1](module-09/ex1/alien_contact.py), [ex2](module-09/ex2/space_crew.py) | Pydantic, BaseModel, Field, validation rules
| [10](module-10) | [ex0](module-10/ex0/lambda_spells.py) | lambda functions
|    | [ex1](module-10/ex1/higher_magic.py) | higher-order functions
|    | [ex2](module-10/ex2/scope_mysteries.py) | lexical scope, closures, nested functions, mutable state retention
|    | [ex3](module-10/ex3/functools_artifacts.py) | functools: reduce, partial, lru_cache, singledispatch
|    | [ex4](module-10/ex4/decorator_mastery.py) | decorator, decorator factories, functools.wraps

## Flake8 / Mypy
All these exercises comply with **flake8** and **mypy strict**. Flake8 is a linter tool that reports styling and coding issues. Mypy checks that type annotations are used when they are needed. Comply with flake8 and mypy is mandatory for 42 code. Comply with mypy strict is recommended. To use them, install the two modules entering this command:
```
python3 -m pip install flake8 mypy
```

To run them in your currect directory, enter:
```
python3 -m flake8
python3 -m mypy --strict .
```

I reccomend adding alias (shortcuts) to run both faster. To do so, enter the command: `vim ~/.zshrc` and at the end of the file, add:
```
alias flake8='python3 -m flake8'
alias mypy='python3 -m mypy'
alias flmy='python3 -m flake8 . && python3 -m mypy --strict .'
```

With these alias, after restarting the terminal, you only need to enter `flake8` or `mypy --strict .` to run them. Or even easier, enter `flmy` to run both, one after the other.